function [dFoF, Fo, img] = normalizeImg(img, percentile,sampRate, bleachCorrectFlag)
...
%   resulting image is then divided by Fo. 
    ...
    %sampRate = info.TimeStamps.AvgStep; %sampling rate in Hz
    [m,n,T] = size(img);
...
%     movie = double(img);
%     moviebl = squeeze(prctile(permute(movie,[3 1 2]),10));
%     movie = (movie-moviebl)./moviebl;
%     writeTif(uint16(movie*2000),'dfof.tif',16);
