function generateActivityMovie_IHCs(img,activeROI,eventStart,allROI,name,clims)
...
%                 name = name of file to output
    ...
    for i = 1:size(img,3)
        imagesc(img(:,:,i));
        if i == eventStart(j)
           activeHC = [activeROI(j).event];
           for x = 1:length(activeHC)
               mask = [mask+allROI{1,activeHC(x)}];
           end
