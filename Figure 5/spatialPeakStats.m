...
function [eventStat] = eventStats(rightEvents, leftEvents, biEvents, smRIC, smLIC, leftBinary, rightBinary)
    ...
    eventStat = struct('eventClassification',{},'leftOrRightDom', {}, 'numPeaks', [], 'domAmp', [], 'maxLamp', [], 'maxRAmp', [],...
        'xloc', [], 'tloc', [],'hwx', [], 'hwt', [], 'integral', [],'integralnorm', [],'lxloc',[],'rxloc',[],'delta',[]);
    index = 1;
    ...
    for i=1:max(leftEvents)
        ...
        if size(r) == 1
            ...
            eventStat(index).integralnorm = trapz(smLIC(:,c)./max(smLIC(:,c)));
...
%             end   
    ...
    for i=1:max(rightEvents)
        ...
        if size(r) == 1
            ...
            eventStat(index).integralnorm = trapz((smRIC(:,c)./max(smRIC(:,c))));
...
%             end   
    ...
    for i=1:max(biEvents)
        ...
        if leftAmp > rightAmp
          ...
          eventStat(index).integralnorm = trapz((smLIC(:,lc)./max(smLIC(:,lc))));
          ...
          rightInd = 100;
           ...
           eventStat(index).integralnorm = trapz((smRIC(:,rc)./max(smRIC(:,rc))));
