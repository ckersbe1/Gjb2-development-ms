...
for i=1:length(file.ISCstruct.event)
    ...
    cm = hsv;
    hold on;
   plot(file.ISCstruct.posInd(file.ISCstruct.event(i).startLoc,1:2:end),file.ISCstruct.posInd(file.ISCstruct.event(i).startLoc,2:2:end),'Color',cm(ceil(locs(i)/numFrames*256),:));
end

%h2 = colorTicks(locs,numFrames);
